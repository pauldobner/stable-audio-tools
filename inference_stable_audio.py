...
# Constants
MODEL_CONFIG_PATH = "runs/audiocaps_finetune_ctrl/3qfv6n0i/checkpoints/model_config_small_custom.json"  # "hf_model_download/model_config_small_original.json"
MODEL_CKPT_PATH = "runs/audiocaps_finetune_ctrl/3qfv6n0i/checkpoints/epoch=2-step=50000.ckpt"  # "hf_model_download/model_small.safetensors"
...
SEED = 42
TTA = True  # Set to True for Text-to-Audio, False for Style Transfer
...
if __name__ == "__main__":
    ...
    if not os.path.exists(dataset_root):
        ...
        print(
            f"Warning: Dataset root {dataset_root} does not exist. Please check configuration."
        )

    dataset = VimSketchDataset(dataset_root)

    print(f"Dataset loaded with {len(dataset)} valid imitation-reference pairs.")

    if TTA:
        save_path = os.path.join(dataset_root, "tta_sao_ctrl_finetune", "audios")
    else:
    ...
    if os.path.exists(checkpoint_file):
        with open(checkpoint_file, "r") as f:
            ...
            logging.info(
                f"Resuming from index {start_idx} ({start_idx}/{len(dataset)} files)"
            )
    ...
    try:
        ...
        if os.path.exists(cache_file):
            try:
                with open(cache_file, "r") as f:
                    ...
                    logging.info(
                        f"Loaded {len(tta_cache)} valid entries from cache file"
                    )
        ...
        def save_cache(force=False):
            try:
                ...
                for (text, duration), file_path in tta_cache.items():
                    cache_data.append(
                        {"text": text, "duration": duration, "file_path": file_path}
                    )
            ...
            try:
                ...
                if TTA:
                    ...
                    if (text, duration) in tta_cache:
                        ...
                        if os.path.exists(outfile_path_cache):
                            logging.info(
                                f"Copying {imitation_filename}, was already processed"
                            )
                            shutil.copy(outfile_path_cache, output_file)
                            # Update checkpoint
                            with open(checkpoint_file, "w") as f:
                                json.dump(
                                    {"last_processed_idx": i, "timestamp": time.time()},
                                    f,
                                )
                            continue
                ...
                if (i - start_idx) % 5 == 0 or i == len(dataset) - 1:
                    ...
                    if processed > 0:
                        ...
                        logging.info(
                            f"Processed {processed} files. Progress: {i + 1}/{len(dataset)}"
                        )
                        logging.info(
                            f"Time elapsed: {elapsed_str}, Est. remaining: {remaining_str}"
                        )
        ...
        logging.info(
            f"Process finished or interrupted. Total runtime: {str(timedelta(seconds=int(total_time)))}"
        )
